...
export function Anamnesis() {
  ...
  const handleSubmit = async (values) => {
    try {
      console.log(values);
      const { error } = await supabase.from("anamnesis").insert(values);
        ...
        <Container justify="center">
          <Paper shadow="xs" mt="xl" p="md">
            <Flex justify="center" align="center" direction="column" gap="md">
              ...
              />
              <Text>
                Vaši anamnézu jsme v pořádku přijali. Děkujeme za vyplnění.
              </Text>
              <Button color="#4FC4E3" onClick={() => navigate("/clients")}>
          ...
          <Stepper color="#4FC4E3" active={active}>
            ...
            <Stepper.Step label="Anamnéza" description="Zdravotní informace">
              ...
              <Select
                ...
                data={[
                  "Ne",
                  "Ano, méně než 10 cigaret denně",
                  "Ano, více než 10 cigaret denně",
                ]}
            ...
            <Stepper.Step
              ...
              description="Aktuální léčba, obtíže"
            >
              <Textarea
            ...
            <Stepper.Completed>
              Děkujeme za aktualizaci Vaší lékařské anamnézy. Pečlivě si prosím
              zkontrolujte vyplněné údaje a případně potvrďte odeslání tlačítkem{" "}
              <strong>Odeslat</strong>:
              <Paper shadow="xs" mt="xl" p="md">
                <Flex direction="column" gap="xs">
                  <Text>
                    <strong>Jméno a příjmení:</strong> {form.values.name}
                  </Text>
                  <Text>
                    <strong>Rok narození:</strong> {form.values.birthDate}
                  </Text>
                  <Text>
                    <strong>Adresa:</strong> {form.values.address}
                  </Text>
                  <Text>
                    <strong>Email:</strong> {form.values.email}
                  </Text>
                  <Text>
                    <strong>Telefon:</strong> {form.values.phone}
                  </Text>
                  <Text>
                    <strong>Rodinná anamnéza:</strong>{" "}
                    {form.values.familyHistory}
                  </Text>
                  <Text>
                    <strong>Chronické nemoci:</strong>{" "}
                    {form.values.chronicDiseases}
                  </Text>
                  <Text>
                    <strong>Pravidelné léky:</strong> {form.values.medications}
                  </Text>
                  <Text>
                    <strong>Alergie:</strong> {form.values.allergies}
                  </Text>
                  <Text>
                    <strong>Kouření:</strong> {form.values.smoking}
                  </Text>
                  <Text>
                    <strong>Konzumace alkoholu:</strong> {form.values.alcohol}
                  </Text>
                  <Text>
                    <strong>Cvičení:</strong> {form.values.exercise}
                  </Text>
                  <Text>
                    <strong>Aktuální zdravotní potíže:</strong>{" "}
                    {form.values.currentIssues}
                  </Text>
                  <Text>
                    <strong>Příznaky:</strong> {form.values.symptoms}
                  </Text>
                </Flex>
              </Paper>
              {errorText === null ? (
                ...
                <Alert variant="light" color="red" title="Chyba" my="md">
                  Chyba při odesílání. Zkontrolujte připojení k internetu a
                  zkuste to prosím později.<br></br>
                  <br></br>
                  {errorText}
                </Alert>
              )}
